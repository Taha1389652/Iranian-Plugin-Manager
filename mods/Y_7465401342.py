...
# Party Window Scale
PARTY_WINDOW_SCALE_SMALL  = 0.845
PARTY_WINDOW_SCALE_MEDIUM = 0.645
PARTY_WINDOW_SCALE_LARGE  = 0.565
...
party_window_folder_name = 'FluffysResponderPartyWindow'
...
config_party_file_name = 'FluffysPartyWindowSettings'
...
#@@@@@@@@@@@@@@@# AUTO RESPONDER DATA #@@@@@@@@@@@@@@@#
######## Chats Data ########
responder_folder_name = 'FluffysAutoResponder'
...
config_responder_file_name = 'FluffysResponderSettings'
...
'savePlayerDataHappyMsg2': {
    'id': 'Daymm, Kamu Mengumpulkan {} Nama Pemain! Fluffy Senang xD',
    'en': 'Daymm, You Collected {} Player Names! Fluffy is Happy xD',
    'hi': 'Daymm, aapne {} player ke naam ikatthe kiye hain! Fluffy khush hai xD',
    'es': '¡Vaya, has recopilado {} nombres de jugadores! Fluffy está feliz xD',
    'ml': 'Daymm, ningal {} playernte perukal collect cheythu! Fluffy santhoshappettu xD'
},
...
'translatePopupWarning': {
    'id': 'Fitur ini tidak dilanjutkan/diuji oleh Fluffy\nDapat menyebabkan error yang tak terduga\nGunakan dengan resiko mu sendiri',
    'en': 'This feature is not continued/tested by Fluffy\nMay cause unexpected errors\nUse at your own risk',
    'hi': 'Ye feature Fluffy dwara na to jari hai na hi test kiya gaya hai\nAnapekshit errors ka karan ban sakta hai\nApne risk par upyog karein',
    'es': 'Esta característica no es continuada/probada por Fluffy\nPuede causar errores inesperados\nÚsalo bajo tu propio riesgo',
    'ml': 'Ee feature Fluffy kooduthal test cheyyappetta onnumalla\nEttavum pratheekshikkatha errors undaakum\nSwantam riskil upayogikkuka'
},
'pswTitle': {
...
config_name_enable_fluffy_responder         = 'enable'
...
default_responder_config = {
    config_name_enable_fluffy_responder: False,
...
"""Global Fluffys Responder Settings,
...
def get_responder_config_translate_text() -> dict[str, str]:
    config_display_names = {
        config_name_enable_fluffy_responder: get_lang_text('rswEnable'),
        ...
        config_name_fluffy: get_lang_text('rswAutoGreet'),
...
CFG_NAME_PLUGIN_FRESH_USAGE_TIME = "FluffyPartyWindow Fresh Usage Time"
...
"""Global Fluffys Party Window Main Settings,
...
def save_blacklist_names(blacklist: List[str]) -> None:
    ...
    try:
        for name in MY_MASTER+[f"{babase.charstr(babase.SpecialChar.V2_LOGO)}FluffyPal"]:
...
def save_muted_players(muted_players: List[str]) -> None:
    ...
    for name in muted_players:
        if name in MY_MASTER+[f"{babase.charstr(babase.SpecialChar.V2_LOGO)}FluffyPal"]:
    ...
    try:
        if os.path.exists(exception_for_player_names_file_path):
            with open(exception_for_player_names_file_path, 'r', encoding='utf-8') as f:
                ...
                if not any(name in "FluffyPal" for name in players_anti_abuse_exception):
                    players_anti_abuse_exception.append("FluffyPal")
                    updated = True
            ...
            return MY_MASTER if any(name in "FluffyPal" for name in MY_MASTER) else MY_MASTER + ['FluffyPal']
    except Exception as e:
...
def sanitize_name(name: str):
    """Accurate Player's `Icon` Remover, By FluffyPal"""
...
if not all(name and name[1:].isalnum() for name in MY_MASTER):
    ...
    raise NameError(msg.removeprefix(__name__+PNAME_AND_MSG_SPLITTER))
"""
if not 'fluffy' in __name__.lower():
    msg = f"{__name__}{PNAME_AND_MSG_SPLITTER}Hey, You Need To Add \"Fluffy\" Word In Fluffy\'s PartyWindow Plugin\'s Name, Kindly Support Me :)"
    screenmessage(msg, COLOR_SCREENCMD_ERROR)
    raise NameError(msg.removeprefix(__name__+PNAME_AND_MSG_SPLITTER))
...
class FluffysPartyWindow(bauiv1lib.party.PartyWindow):
    """
    Fluffys Party Window :3

    Combined/Inspired from random modders idea
    """
    ...
    def __init__(self, origin: Sequence[float] = (0, 0)) -> None:
        ...
        self._width  : int = (1350 if self.uiscale is babase.UIScale.SMALL else
                              1200 if self.uiscale is babase.UIScale.MEDIUM else
                              1100) # Lebar
        ...
        if host_info.get('name'):
            ...
            self.title = f"{get_lang_text('partyWindow.titleText')} :3"
        ...
        self._thanks_button = bui.buttonwidget(
            ...
            button_type='square',
            color=self.bg_color,
            position=(self._width-25, self._height * 0.7 if not self.uiscale is babase.UIScale.SMALL else self._height * 0.645),
        )
        bui.buttonwidget(
            ...
            for y in range(rows):
                for x in range(columns):
                    ...
                    if index < len(self._roster):
                        ...
                        elif p_acc in f"{babase.charstr(babase.SpecialChar.V2_LOGO)}FluffyPal":
...
class FluffysAutoResponder:
    ...
    def master_chats(self, msg: Optional[str] = None):
        ...
        if not responder_config.get(config_name_enable_fluffy_responder): return
        ...
        if self.nama_pemain not in player_muted_list:
            ...
            if responder_config.get(config_name_fluffy):
                Thread(target=self.fluffy).start()
    ...
    def fluffy(self):
        ...
        if (any(word in self.jawab_lain_lower for word in list_kata_halo) and 
            ...
            if sapaan and not is_sapaan_my_master:
                ...
                babase.pushcall(Call(refresh_job, fluffy_respone_delay, defname), from_other_thread=True)
...
class ResponderSettingsWindow(popup.PopupWindow):
    ...
    def _apply_setting(self, display_name: str, value: bool) -> None:
        """Apply a specific setting immediately without recalling FluffysAutoResponder"""
...
def _get_bcs_player_info(name: str):
    ...
    # Code Get From Sara, Edited By FluffyPal
...
class by_FluffyPal(babase.Plugin):

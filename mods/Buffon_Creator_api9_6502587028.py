...
def _normalized_color(color: Sequence[float]) -> Sequence[float]:
    """Return a brightness-normalized copy of a color.

    api9 no longer exposes the old private ba._math.normalized_color
    helper, so this is a small local re-implementation used just for
    picking a nice light color for the spawn flash.
    """
    total = sum(color)
    if total < 0.01:
        return (1.0, 1.0, 1.0)
...
class SoccerGame(HockeyGame):
    ...
    def spawn_player(self, player: Player) -> bs.Actor:
        ...
        light_color = _normalized_color(color)
    ...
    def _spawn_puck(self) -> None:
        ...
        if self._auto_shoot:
            ...
            shoot_delay = 0.5
            bs.timer(shoot_delay, bs.WeakCall(self._shoot_puck, this_puck, shot))
            ...
            bs.timer(reset_delay, bs.WeakCall(self._check_force_reset, this_puck))
    ...
    def _shoot_puck(self, puck: Puck, shot: Dict[str, Any]) -> None:
        ...
        bs.timer(0.28, bs.WeakCall(self._home_puck, puck, speed, 1))
        bs.timer(0.55, bs.WeakCall(self._home_puck, puck, speed, 2))
        bs.timer(0.85, bs.WeakCall(self._home_puck, puck, speed, 3))

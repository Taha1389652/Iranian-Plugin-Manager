...
"""
============================================================================
  نمایشگر پینگ لحظه‌ای در پنجره پارتی (Party / Chat Window) — BombSquad
============================================================================

این مود، پنجره‌ی استاندارد پارتی/چت بازی (همون پنجره‌ای که با دکمه شبکه باز
میشه و اسم بازیکن‌ها و باکس چت توش هست) رو با یک نسخه‌ی جدید جایگزین می‌کنه که
دقیقاً همون ظاهر و رفتار اصلی بازی رو داره، به‌علاوه‌ی یک دکمه‌ی تازه بالا-راست
پنجره (کنار دکمه‌ی «...») به اسم «پینگ».

با زدن این دکمه، زیر اسم هر بازیکن توی لیست پارتی، عدد پینگ اون به میلی‌ثانیه
نمایش داده میشه و هر ثانیه به‌صورت زنده (لحظه‌ای) آپدیت میشه. رنگ عدد هم بر اساس
کیفیت پینگ عوض میشه:
    سبز   -> پینگ عالی   (زیر ۸۰ میلی‌ثانیه)
    زرد   -> پینگ خوب    (۸۰ تا ۱۵۰)
    نارنجی -> پینگ متوسط  (۱۵۰ تا ۲۵۰)
    قرمز  -> پینگ بد      (بالای ۲۵۰)
    خاکستری/-- -> پینگ در دسترس نیست

نکته‌ی مهم درباره‌ی API بازی (این محدودیت خودِ موتور بازیه، نه این مود):
موتور Ballistica فقط یک تابع عمومی برای خوندن پینگ در اختیار می‌ذاره:
`bascenev1.get_client_ping(client_id)` که RTT (رفت‌وبرگشت) یک کلاینتِ متصل رو
برمی‌گردونه. این تابع فقط از دید «میزبان» (host) یک پارتی معنی داره، یعنی وقتی
شما خودتون سرور/پارتی رو ران کردید (دقیقاً همون کاری که برای مودهای آفلاین و
سرورهای شخصی‌تون انجام می‌دید). اگر به عنوان کلاینت به پارتیِ یک نفر دیگه وصل
بشید، موتور بازی راهی برای خوندن پینگ خودتون به شما نمیده، برای همین در اون
حالت به‌جای عدد، «--» نمایش داده میشه (به‌جای کرش کردن یا نمایش عدد غلط).

نصب: این فایل رو مستقیم داخل پوشه‌ی mods بازی کپی کنید (همون جایی که مودهای
دیگه‌تون مثل infinite boxing gloves رو گذاشتید).
============================================================================
"""
...
import bauiv1lib.party as _party_module
...
# ---------------------------------------------------------------------------
# رنگ‌های نمایش کیفیت پینگ — هر جور دوست داشتید اینجا عوضشون کنید
# ---------------------------------------------------------------------------
_PING_COLOR_GREAT = (0.35, 1.0, 0.35)      # سبز — عالی
_PING_COLOR_GOOD = (0.85, 1.0, 0.35)       # زرد‌مایل‌به‌سبز — خوب
_PING_COLOR_OK = (1.0, 0.75, 0.25)         # نارنجی — متوسط
_PING_COLOR_BAD = (1.0, 0.35, 0.35)        # قرمز — بد
_PING_COLOR_UNKNOWN = (0.65, 0.65, 0.65)   # خاکستری — نامشخص

_PING_BTN_COLOR_OFF = (0.30, 0.45, 0.65)
_PING_BTN_COLOR_ON = (0.35, 0.85, 0.35)


def _ping_color(ping_ms: float | None) -> tuple[float, float, float]:
    """رنگ مناسب رو بر اساس مقدار پینگ (میلی‌ثانیه) برمی‌گردونه."""
    if ping_ms is None:
        return _PING_COLOR_UNKNOWN
    if ping_ms <= 80:
        return _PING_COLOR_GREAT
    if ping_ms <= 150:
        return _PING_COLOR_GOOD
    if ping_ms <= 250:
        return _PING_COLOR_OK
    return _PING_COLOR_BAD


class PingPartyWindow(bui.Window):
    """نسخه‌ی ارتقایافته‌ی پنجره پارتی/چت بازی به‌همراه نمایشگر پینگ لحظه‌ای."""
    ...
    def __init__(self, origin: Sequence[float] = (0, 0)):
        ...
        # وضعیت نمایش/عدم‌نمایش پینگ (پیش‌فرض: خاموش تا پنجره شلوغ نشه)
        ...
        super().__init__(
            ...
            # داخل overlay_stack هستیم تا با بازسازی پنجره‌های اصلی از بین نریم
        ...
        # =====================================================================
        # دکمه‌ی جدید: نمایش/مخفی‌کردن پینگ لحظه‌ای
        # دقیقاً کنار دکمه‌ی منو («...») در بالا-راست پنجره پارتی قرار می‌گیره
        # =====================================================================
        self._ping_toggle_button = bui.buttonwidget(
            parent=self._root_widget,
            scale=0.7,
            position=(self._width - 116, self._height - 47),
            size=(50, 50),
            label=bui.Lstr(value='پینگ'),
            autoselect=True,
            button_type='square',
            on_activate_call=bui.WeakCall(self._on_ping_toggle_press),
            color=_PING_BTN_COLOR_OFF,
        )
        bui.widget(edit=self._menu_button, left_widget=self._ping_toggle_button)
        bui.widget(edit=self._ping_toggle_button, right_widget=self._menu_button)
        ...
        # ویجت‌های برچسبِ پینگ + شناسه‌ی کلاینتِ متناظر با هر ردیف روستر
        self._ping_widgets: dict[int, bui.Widget] = {}
        self._roster_client_ids: dict[int, int | None] = {}

        self._roster: list[dict[str, Any]] | None = None
        self._update_timer = bui.AppTimer(
            1.0, bui.WeakCall(self._update), repeat=True
        )
        # تایمر جدا و مستقل برای آپدیت لحظه‌ای (هر ثانیه) مقدار عددی پینگ‌ها
        # این تایمر جدا از تایمر بالا کار می‌کنه تا حتی وقتی روستر عوض نشده
        # (کسی جوین/لیو نکرده) بازم عدد پینگ هر ثانیه تازه بشه.
        self._ping_update_timer = bui.AppTimer(
            1.0, bui.WeakCall(self._update_ping_values), repeat=True
        )
        self._update()

    # -------------------------------------------------------------------
    # منطق دکمه و نمایشگر پینگ
    # -------------------------------------------------------------------
    def _on_ping_toggle_press(self) -> None:
        """با فشردن دکمه‌ی «پینگ»، نمایش لحظه‌ای پینگ روشن/خاموش میشه."""
        self._ping_visible = not self._ping_visible
        try:
            bui.getsound('click01').play()
        except Exception:
            pass
        bui.buttonwidget(
            edit=self._ping_toggle_button,
            color=_PING_BTN_COLOR_ON if self._ping_visible else _PING_BTN_COLOR_OFF,
        )
        # فوراً مقادیر رو به‌روز کن؛ لازم نیست یک ثانیه صبر کنیم
        self._update_ping_values()

    def _update_ping_values(self) -> None:
        """هر ثانیه صدا زده میشه؛ عدد پینگِ کنار اسم هر بازیکن رو تازه می‌کنه."""
        for index, widget in self._ping_widgets.items():
            if not widget:
                continue
            if not self._ping_visible:
                bui.textwidget(edit=widget, text='')
                continue

            client_id = self._roster_client_ids.get(index)
            ping_ms: float | None = None
            if client_id is not None:
                try:
                    ping_ms = bs.get_client_ping(client_id)
                except Exception:
                    # یا این تابع در این نسخه از بازی وجود نداره، یا ما
                    # میزبان پارتی نیستیم و اجازه‌ی خوندن این مقدار رو نداریم
                    ping_ms = None

            text = '--' if ping_ms is None else f'{int(round(ping_ms))}ms'
            bui.textwidget(edit=widget, text=text, color=_ping_color(ping_ms))

    # -------------------------------------------------------------------
    # از این پایین، منطق اصلیِ پنجره پارتی است (بدون تغییر نسبت به بازی اصلی)
    # -------------------------------------------------------------------
    ...
    def _update(self) -> None:
        ...
        if roster != self._roster:
            ...
            # پاک‌سازی ویجت‌های پینگِ قدیمی، چون روستر عوض شده
            for widget in self._ping_widgets.values():
                widget.delete()
            self._ping_widgets = {}
            self._roster_client_ids = {}
                ...
                for y in range(rows):
                    for x in range(columns):
                        ...
                        if index < len(self._roster):
                            ...
                            # =========================================
                            # برچسبِ پینگِ لحظه‌ای، درست زیر نام بازیکن
                            # مقدارش توسط self._update_ping_values() هر
                            # ثانیه تازه میشه (این‌جا فقط ساخته میشه)
                            # =========================================
                            ping_widget = bui.textwidget(
                                parent=self._root_widget,
                                position=(pos[0] + 2, pos[1] - 11),
                                scale=0.38,
                                size=(0, 0),
                                color=_PING_COLOR_UNKNOWN,
                                shadow=0.2,
                                flatness=1.0,
                                h_align='left',
                                v_align='center',
                            ...
                            self._ping_widgets[index] = ping_widget
                            self._roster_client_ids[index] = self._roster[
                                index
                            ]['client_id']
            ...
            # روستر تازه ساخته شد؛ بلافاصله مقادیر پینگ رو هم پر کن
            self._update_ping_values()
...
def _apply_patch() -> None:
    """پنجره‌ی پارتی استاندارد بازی رو با نسخه‌ی دارای نمایشگر پینگ عوض می‌کنه."""
    _party_module.PartyWindow = PingPartyWindow


# پچ رو همین الان (در لحظه‌ی import شدن فایل) اعمال کن
_apply_patch()


# ba_meta export babase.Plugin
class PingDisplayPlugin(babase.Plugin):
    """پلاگین نگه‌دارنده‌ی مود؛ کارِ اصلی همون تعویضِ PartyWindow هست."""

    def on_app_running(self) -> None:
        # برای اطمینان کامل، پچ رو دوباره هم موقع اجرا شدن بازی اعمال می‌کنیم
        _apply_patch()

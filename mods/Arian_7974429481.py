...
class SoccerGame(HockeyGame):
    ...
    description = 'Server: Arian | By: Ali / @Bombsquad_Mod1'
    available_settings = [
        ba.IntSetting(
            'Score to Win',
            min_value=1,
        ...
        ba.BoolSetting('بات تمرینی (شوت خودکار)', default=False),
    ...
    def __init__(self, settings: dict):
        ...
        self._training_bot = bool(settings.get('بات تمرینی (شوت خودکار)', False))
        self._training_bot_timer: Optional[ba.Timer] = None
    ...
    def on_begin(self) -> None:
        ...
        if self._training_bot:
            self._start_training_bot_loop()
    ...
    def _spawn_puck(self) -> None:
        ...
        if self._auto_shoot or self._training_bot:
            ba.timer(0.35, ba.WeakCall(self._do_auto_shoot))

    def _do_auto_shoot(self) -> None:
        """Fire the puck hard toward the goal right after it spawns."""
        import math
        if self._puck is None or self._puck.node is None:
            return
        target = self._goal_target_pos
        pos = self._puck.node.position
        dx = target[0] - pos[0]
        dy = target[1] - pos[1]
        dz = target[2] - pos[2]
        ...
        vy = 3.0
        vz = (dz / dist) * speed
        self._puck.node.velocity = (vx, vy, vz)
        ba.playsound(self._swipsound)

    def _start_training_bot_loop(self) -> None:
        """Start the practice-bot helper: a recurring timer that keeps
        feeding the ball toward the goal automatically, so solo/training
        play never stalls waiting on a human kickoff."""
        self._training_bot_timer = ba.Timer(
            5.0, ba.WeakCall(self._training_bot_tick), repeat=True)

    def _training_bot_tick(self) -> None:
        """Practice-bot behavior: whenever the ball has come to a stop,
        automatically shoot it toward the goal again."""
        if not self._training_bot:
            return
        if self._puck is None or self._puck.node is None:
            return
        vx, vy, vz = self._puck.node.velocity

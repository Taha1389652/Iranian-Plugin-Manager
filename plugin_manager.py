...
REPOSITORY_URL = "https://github.com/Taha1389652/Iranian-Plugin-Manager"
CURRENT_TAG = "main"
...
class StartupTasks:
    ...
    async def update_plugin_manager(self):
        ...
        if update_details:
            ...
            bui.screenmessage(f"مدیریت پلاگین در حال به‌روزرسانی به نسخه {to_version}")
            try:
                await self.plugin_manager.update(to_version, commit_sha)
            except MD5CheckSumFailed:
                bui.getsound('error').play()
            else:
                bui.screenmessage("به‌روزرسانی موفقیت‌آمیز بود. برای اعمال تغییرات بازی را مجدداً راه‌اندازی کنید.",
                                  color=(0, 1, 0))
        ...
        if existing_num_of_plugins < new_num_of_plugins:
            ...
            if new_supported_plugins_count > 0:
                ...
                if new_supported_plugins_count == 1:
                    notification_text = f"{new_supported_plugins_count} پلاگین جدید ({new_supported_plugins}) در دسترس است!"
                else:
                    notification_text = new_supported_plugins + \
                        ('' if new_supported_plugins_count <= show_max_names else ' و +' +
                         str(new_supported_plugins_count-show_max_names)) + " پلاگین جدید در دسترس است"
    ...
    def load_minigames(self):
        ...
        for game in scanner.results.exports["bascenev1.GameActivity"]:
            if game not in scanned_results:
                bui.screenmessage(f"{game} مینی‌گیم بارگذاری شد")
    ...
    def unload_minigames(self):
        ...
        for game in babase.app.meta.scanresults.exports["bascenev1.GameActivity"]:
            if game in scanner.results.exports["bascenev1.GameActivity"]:
                bui.screenmessage(f"{game} مینی‌گیم بارگیری شد")
            else:
    ...
    async def enable(self):
        for entry_point in await self.get_entry_points():
            ...
            if plugin_spec not in bui.app.plugins.active_plugins:
                ...
                bui.screenmessage(f"{entry_point} بارگذاری شد")
        if await self.has_minigames():
            self.load_minigames()
        self.save()
    ...
    async def install(self, suppress_screenmessage=False):
        try:
            ...
            if not suppress_screenmessage:
                bui.screenmessage(
                    f"{self.plugin.name} بررسی MD5 در هنگام نصب ناموفق بود", color=(1, 0, 0))
            return False
        else:
            if not suppress_screenmessage:
                bui.screenmessage(f"{self.plugin.name} نصب شد", color=(0, 1, 0))
...
class Plugin:
    ...
    def latest_compatible_version(self):
        ...
        if self._latest_compatible_version is None:
            raise NoCompatibleVersion(
                f"{self.name} هیچ نسخه‌ای سازگار با API {_app_api_version} ندارد."
            )
    ...
    def get_local(self):
        if not self.is_installed:
            raise PluginNotInstalled(
                f"{self.name} باید نصب شود تا پلاگین محلی آن دریافت شود.")
    ...
    async def uninstall(self):
        ...
        bui.screenmessage(f"{self.name} حذف شد", color=(0.9, 1, 0))
    ...
    async def update(self):
        if await self.latest_compatible_version.install(suppress_screenmessage=True):
            bui.screenmessage(f"{self.name} به نسخه {self.latest_compatible_version.number} به‌روزرسانی شد",
                              color=(0, 1, 0))
            bui.getsound('shieldUp').play()
        else:
            bui.screenmessage(f"{self.name} بررسی MD5 در هنگام به‌روزرسانی به نسخه "
                              f"{self.latest_compatible_version.number} ناموفق بود",
                              color=(1, 0, 0))
            bui.getsound('error').play()
        ...
        try:
            ...
            if full_changelog[1]:
                ...
                if matches:
                    ...
                    changelog = {'released_on': ' (ارائه نشده)',
                                 'info': f"ورودی تغییرات برای نسخه {version} یافت نشد."}
            else:
                changelog = full_changelog[0]
        except urllib.error.URLError:
            changelog = {'released_on': ' (ارائه نشده)',
                         'info': 'به دلیل مشکلات اینترنتی امکان دریافت تغییرات وجود ندارد.'}
...
class ChangelogWindow(popup.PopupWindow):
    def __init__(self, origin_widget):
        ...
        try:
            ...
            released_on = ''
            logs = ["امکان بارگذاری تغییرات وجود ندارد"]
            h_align = 'center'
            extra = 1
    ...
    def __init__(self, authors_info, origin_widget):
        ...
        for author in self.authors_info:
            for key, value in author.items():
                text = f"{key.title()}: {value if value != '' else 'ارائه نشده'}"
                if key == 'name':
                    text = value
                bui.textwidget(
...
class PluginWindow(popup.PopupWindow):
    ...
    async def draw_ui(self):
        ...
        # author
        text = 'توسط ' + ', '.join([author["name"] for author in self.plugin.info["authors"]])
        ...
        if tutorial_url:
            ...
            bui.buttonwidget(
                ...
                on_activate_call=bui.CallPartial(
                    ...
                    text=f'این شما را به آدرس زیر می‌برد:\n{tutorial_url}',
...
class MoreWindow:
    def __init__(self, plugin: Plugin, origin=None):
        ...
        bui.textwidget(
            ...
            text=(
                last_updated and
                f'آخرین به‌روزرسانی: {last_updated}'
                or 'نامشخص'
            )
        )
        # version
        ...
        bui.textwidget(
            ...
            max_height=step,
            size=(step, step),
            text=f"نسخه: {plugin.latest_compatible_version.number}"
        )
        # source url
        py += step+margin
        bui.textwidget(
...
```"""
        ...
        params = {
            "title": "[باگ پلاگین]: " + self.plugin.name,
            "plugin-name": self.plugin.name,
            "plugin-version": self.plugin.create_local().version if self.plugin.is_installed else 'نصب نشده',
...
class PluginCustomSourcesWindow(popup.PopupWindow):
    def __init__(self, origin_widget):
        ...
        bui.textwidget(
            ...
            size=(50, 22),
            text=("هشدار: منابع پلاگین شخص ثالث نظارت نمی‌شوند\n"
                  "               و ممکن است خطرناک باشند!"),
            color=(1, 0.23, 0.23),
            scale=0.5,
            h_align="left",
            v_align="center",
    ...
    async def add_source(self):
        ...
        if source in babase.app.config["Community Plugin Manager"]["Custom Sources"]:
            bui.screenmessage("منبع پلاگین قبلاً وجود دارد")
            bui.getsound('error').play()
            return
        babase.app.config["Community Plugin Manager"]["Custom Sources"].append(source)
        babase.app.config.commit()
        bui.screenmessage("منبع پلاگین اضافه شد؛ برای اعمال تغییرات لیست پلاگین را به‌روز کنید",
                          color=(0, 1, 0))
        bui.getsound('cashRegister2').play()
        self.draw_sources()
    ...
    def delete_selected_source(self):
        ...
        babase.app.config.commit()
        bui.screenmessage("منبع پلاگین حذف شد؛ برای اعمال تغییرات لیست پلاگین را به‌روز کنید",
                          color=(0.9, 1, 0))
        bui.getsound('shieldDown').play()
        self.draw_sources()
...
class PluginCategoryWindow(popup.PopupMenuWindow):
    def __init__(self, choices, current_choice, origin_widget, asyncio_callback):
        choices = (*choices, "نصب شده", "منابع سفارشی")
...
class PluginManagerWindow(bui.MainWindow):
        ...
        try:
            ...
            try:
                bui.textwidget(
                    ...
                    text="مطمئن شوید که به اینترنت متصل هستید و دوباره امتحان کنید."
                )
            except:
                pass
    ...
    def draw_category_selection_button(self, post_label):
        ...
        if self.alphabet_order_selection_button is None:
            self.alphabet_order_selection_button = bui.buttonwidget(
                ...
                label='ز - الف' if self.selected_alphabet_order == 'z_a' else 'الف - ز',
    ...
    async def draw_plugin_names(self, category, search_term="", refresh=False, order='a_z'):
        ...
        try:
            ...
            no_internet_text = "مطمئن شوید که به اینترنت متصل هستید و دوباره امتحان کنید."
            if bui.textwidget(query=self._plugin_manager_status_text) != no_internet_text:
                raise CategoryDoesNotExist(f"{category} وجود ندارد.")
            else:
                return
    ...
    async def refresh(self):
        self.cleanup()
        # try:
        #     bui.textwidget(edit=self._plugin_manager_status_text, text="در حال به‌روزرسانی")
        # except:
        #     pass
...
class PluginManagerSettingsWindow(popup.PopupWindow):
    ...
    async def draw_ui(self):
        ...
        bui.textwidget(
            ...
            h_align='center',
            v_align='center',
            text='در توسعه پلاگین‌ها یا این مدیریت پلاگین مشارکت کنید!',
        ...
        if plugin_manager_update_available:
            ...
            update_button_label = f'به‌روزرسانی به نسخه {plugin_manager_update_available[0]}'
        ...
        bui.textwidget(
            ...
            h_align='center',
            v_align='center',
            text=f'مدیریت پلاگین نسخه {PLUGIN_MANAGER_VERSION}',
            scale=text_scale * 0.8,
            color=text_color,
            maxwidth=width * 0.9
        )
        ...
        bui.textwidget(
            ...
            h_align='center',
            v_align='center',
            text=f'نسخه API: {_app_api_version}',
    ...
    async def update(self, to_version=None, commit_sha=None):
        try:
            ...
            bui.screenmessage("بررسی MD5 در هنگام به‌روزرسانی مدیریت پلاگین ناموفق بود", color=(1, 0, 0))
            bui.getsound('error').play()
        else:
            bui.screenmessage("به‌روزرسانی مدیریت پلاگین موفقیت‌آمیز بود", color=(0, 1, 0))
            bui.getsound('shieldUp').play()
            bui.textwidget(
                edit=self._restart_to_reload_changes_text,
                text='به‌روزرسانی اعمال شد!\nبرای اعمال تغییرات بازی را مجدداً راه‌اندازی کنید.'
            )
...
class NewAllSettingsWindow(AllSettingsWindow):
    ...
    def __init__(
        ...
        _b_title(x_offs6, v, pmb, bui.Lstr(value="مدیریت پلاگین"))
        imgw = imgh = 120
        bui.imagewidget(
            parent=self._root_widget,
